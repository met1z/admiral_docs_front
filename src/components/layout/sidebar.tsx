...
import { ListTodo, Loader2, LogOut, MailPlus, X } from 'lucide-react';
...
export function Sidebar({
  ...
  const activeTypeName = useMemo(() => {
    if (isActionRequiredActive) {
      return t('layout.actionRequired');
    }

    return types.find((type) => type.code === activeTypeCode)?.name ?? t('layout.noTypes');
  }, [activeTypeCode, isActionRequiredActive, t, types]);
  ...
  return (
    <>
      <aside
        className={[
          'fixed inset-y-0 left-0 z-40 flex w-72 flex-col border-r border-slate-200 bg-white/95 backdrop-blur-xl',
          'transform-gpu transition-transform duration-300 ease-out will-change-transform',
          isOpen ? 'translate-x-0 shadow-[12px_0_48px_rgba(15,23,42,0.10)]' : '-translate-x-full shadow-none',
        ].join(' ')}
      >
        ...
        <div className="flex-1 overflow-y-auto px-4 py-5">
          <button
            type="button"
            onClick={onOpenActionRequired}
            className={[
              'mb-5 flex w-full items-center gap-3 rounded-[24px] border px-2 py-2 text-left transition-all duration-300',
              isActionRequiredActive
                ? 'border-cyan-300 bg-gradient-to-br from-cyan-50 via-white to-cyan-100 text-cyan-950 shadow-[0_18px_40px_rgba(6,182,212,0.18)]'
                : 'border-cyan-100 bg-cyan-50/60 text-slate-800 hover:border-cyan-200 hover:bg-cyan-50',
            ].join(' ')}
          >
            <div className={[
            ...
            <span className="min-w-0 flex-1 truncate text-sm font-semibold">{t('layout.actionRequired')}</span>
          </button>
      ...
      {isOpen ? (
        <button
          ...
          aria-label={t('layout.closeDrawer')}
          onClick={onClose}
          className="fixed inset-0 z-30 bg-slate-950/25 opacity-100 backdrop-blur-[1px] transition-opacity duration-300 md:hidden"
        />
      ) : null}

      {inviteOpen ? (
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/35 px-4 opacity-100 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_24px_90px_rgba(15,23,42,0.18)] transition-all duration-300 ease-out animate-[dialog-in_240ms_ease-out]">
